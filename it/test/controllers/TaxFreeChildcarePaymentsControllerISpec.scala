...
class TaxFreeChildcarePaymentsControllerISpec
  ...
  withClient { wsClient =>
    ...
    val endpoints = Table(
      ...
      ("link", "/link", nsiLinkAccountsEndpoint, validLinkAccountsRequestPayloads),
      ("balance", "/balance", nsiCheckBalanceEndpoint, validCheckBalanceRequestPayloads),
      ("payment", "/", nsiMakePaymentEndpoint, validLMakePaymentRequestPayloads)
    )
